...
async def danh_sach_proxy(update, context):
    ...
    try:
        ...
        if not txt_files:
            return await update.message.reply_text("📭 <b>Không tìm thấy file .txt nào trong thư mục hiện tại.</b>", parse_mode='HTML')
        ...
        for file in txt_files:
            try:
                ...
                msg += f"📄 <code>{file}</code> - <i>Lỗi đọc file</i>\n"

        msg += "\n▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n💡 <i>Dùng tên file này trong lệnh /add</i>"
        await update.message.reply_text(msg, parse_mode='HTML')
    except Exception as e:
        await update.message.reply_text(f"❌ <b>Lỗi quét thư mục:</b> {str(e)}", parse_mode='HTML')
...
def main():
    ...
    app.add_handler(CommandHandler("proxies", danh_sach_proxy)) # <--- Lệnh mới kiểm tra proxies
    ...
    print("🚀 Bot đã khởi động! (Đã thêm tính năng Check Proxies)")
    app.run_polling()
